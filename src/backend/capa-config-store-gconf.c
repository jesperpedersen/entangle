...
#include "capa-config-store-gconf.h"

#define CAPA_CONFIG_STORE_GCONF_GET_PRIVATE(obj)                               \
    (G_TYPE_INSTANCE_GET_PRIVATE((obj), CAPA_TYPE_CONFIG_STORE_GCONF, CapaConfigStoreGConfPrivate))

struct _CapaConfigStoreGConfPrivate {
    GConfClient *gconf;
};

static void capa_config_store_gconf_interface_init(gpointer g_iface,
                                                   gpointer iface_data);

G_DEFINE_TYPE_EXTENDED(CapaConfigStoreGConf, capa_config_store_gconf, G_TYPE_OBJECT, 0,
                       G_IMPLEMENT_INTERFACE(CAPA_TYPE_CONFIG_STORE, capa_config_store_gconf_interface_init));


static void capa_config_store_gconf_finalize(GObject *object)
{
    CapaConfigStoreGConf *config = CAPA_CONFIG_STORE_GCONF(object);
    CapaConfigStoreGConfPrivate *priv = config->priv;

    CAPA_DEBUG("Finalize config %p", object);

    g_object_unref(priv->gconf);

    G_OBJECT_CLASS (capa_config_store_gconf_parent_class)->finalize (object);
}


static void capa_config_store_gconf_class_init(CapaConfigStoreGConfClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->finalize = capa_config_store_gconf_finalize;

    g_type_class_add_private(klass, sizeof(CapaConfigStoreGConfPrivate));
}


CapaConfigStoreGConf *capa_config_store_gconf_new(void)
{
    return CAPA_CONFIG_STORE_GCONF(g_object_new(CAPA_TYPE_CONFIG_STORE_GCONF, NULL));
}


static void capa_config_store_gconf_init(CapaConfigStoreGConf *store)
{
    CapaConfigStoreGConfPrivate *priv;

    priv = store->priv = CAPA_CONFIG_STORE_GCONF_GET_PRIVATE(store);
    memset(priv, 0, sizeof(*priv));
}


static gchar *capa_config_store_gconf_make_key(CapaConfigStore *store G_GNUC_UNUSED, CapaConfigSet *set, CapaConfigEntry *entry)
{
    return g_strdup_printf("/apps/capa/%s/%s",
                           capa_config_set_get_name(set),
                           capa_config_entry_get_name(entry));
}


static gboolean capa_config_store_gconf_has_set_entry(CapaConfigStore *store, CapaConfigSet *set, CapaConfigEntry *entry)
{
    CapaConfigStoreGConf *gconf = CAPA_CONFIG_STORE_GCONF(store);
    CapaConfigStoreGConfPrivate *priv = CAPA_CONFIG_STORE_GCONF_GET_PRIVATE(gconf);
    gchar *key = capa_config_store_gconf_make_key(store, set, entry);
    GConfValue *val;
    gboolean has;
...
static gboolean capa_config_store_gconf_clear_set_entry(CapaConfigStore *store, CapaConfigSet *set, CapaConfigEntry *entry)
{
    CapaConfigStoreGConf *gconf = CAPA_CONFIG_STORE_GCONF(store);
    CapaConfigStoreGConfPrivate *priv = CAPA_CONFIG_STORE_GCONF_GET_PRIVATE(gconf);
    gchar *key = capa_config_store_gconf_make_key(store, set, entry);
...
static gboolean capa_config_store_gconfvalue_to_gvalue(int datatype, GConfValue *gconfvalue, GValue *gvalue)
{
    switch (datatype) {
    case CAPA_CONFIG_ENTRY_STRING:
        g_value_set_string(gvalue, gconf_value_get_string(gconfvalue));
        break;

    case CAPA_CONFIG_ENTRY_INT:
        g_value_set_int(gvalue, gconf_value_get_int(gconfvalue));
        break;

    case CAPA_CONFIG_ENTRY_DOUBLE:
        g_value_set_double(gvalue, gconf_value_get_float(gconfvalue));
        break;

    case CAPA_CONFIG_ENTRY_BOOL:
        g_value_set_boolean(gvalue, gconf_value_get_bool(gconfvalue));
        break;


    case CAPA_CONFIG_ENTRY_STRING_LIST: {
    ...
    case CAPA_CONFIG_ENTRY_INT_LIST: {
        GSList *gconflist = gconf_value_get_list(gconfvalue);
    ...
    case CAPA_CONFIG_ENTRY_DOUBLE_LIST:{
        GSList *gconflist = gconf_value_get_list(gconfvalue);
    ...
    case CAPA_CONFIG_ENTRY_BOOL_LIST:{
        GSList *gconflist = gconf_value_get_list(gconfvalue);
    ...
    case CAPA_CONFIG_ENTRY_STRING_HASH: {
        GSList *gconflist = gconf_value_get_list(gconfvalue);
    ...
    case CAPA_CONFIG_ENTRY_INT_HASH: {
        GSList *gconflist = gconf_value_get_list(gconfvalue);
        GHashTable *ghash = g_hash_table_new(g_str_hash, g_str_equal);
        while (gconflist) {
            GConfValue *gconflistvalue = gconflist->data;
    ...
    case CAPA_CONFIG_ENTRY_DOUBLE_HASH: {
        GSList *gconflist = gconf_value_get_list(gconfvalue);
        GHashTable *ghash = g_hash_table_new(g_str_hash, g_str_equal);
        while (gconflist) {
            GConfValue *gconflistvalue = gconflist->data;
    ...
    case CAPA_CONFIG_ENTRY_BOOL_HASH: {
        GSList *gconflist = gconf_value_get_list(gconfvalue);
        GHashTable *ghash = g_hash_table_new(g_str_hash, g_str_equal);
        while (gconflist) {
            GConfValue *gconflistvalue = gconflist->data;
...
static gboolean capa_config_store_gconf_read_set_entry(CapaConfigStore *store, CapaConfigSet *set, CapaConfigEntry *entry, GValue *value)
{
    CapaConfigStoreGConf *gconf = CAPA_CONFIG_STORE_GCONF(store);
    CapaConfigStoreGConfPrivate *priv = CAPA_CONFIG_STORE_GCONF_GET_PRIVATE(gconf);
    gchar *key = capa_config_store_gconf_make_key(store, set, entry);
    ...
    if (gconfvalue) {
        ret = capa_config_store_gconfvalue_to_gvalue(capa_config_entry_get_datatype(entry), gconfvalue, value);
...
static gboolean capa_config_store_gvalue_to_gconfvalue(int datatype, GValue *gvalue, GConfValue **gconfvalue)
{
    switch (datatype) {
    case CAPA_CONFIG_ENTRY_STRING:
...
static gboolean capa_config_store_gconf_write_set_entry(CapaConfigStore *store, CapaConfigSet *set, CapaConfigEntry *entry, GValue *value)
{
    CapaConfigStoreGConf *gconf = CAPA_CONFIG_STORE_GCONF(store);
    CapaConfigStoreGConfPrivate *priv = CAPA_CONFIG_STORE_GCONF_GET_PRIVATE(gconf);
    gchar *key = capa_config_store_gconf_make_key(store, set, entry);
    ...
    ret = capa_config_store_gvalue_to_gconfvalue(capa_config_entry_get_datatype(entry), value, &gconfvalue);
...
{
    CapaConfigStoreInterface *iface = g_iface;

    iface->store_has_set_entry = capa_config_store_gconf_has_set_entry;
    iface->store_clear_set_entry = capa_config_store_gconf_clear_set_entry;
    iface->store_read_set_entry = capa_config_store_gconf_read_set_entry;
    iface->store_write_set_entry = capa_config_store_gconf_write_set_entry;
}
