...
#define CAPA_THUMBNAIL_LOADER_GET_PRIVATE(obj)                                     \
    (G_TYPE_INSTANCE_GET_PRIVATE((obj), CAPA_TYPE_THUMBNAIL_LOADER, CapaThumbnailLoaderPrivate))


struct _CapaThumbnailLoaderPrivate {
    guint width;
    guint height;
};

G_DEFINE_TYPE(CapaThumbnailLoader, capa_thumbnail_loader, CAPA_TYPE_PIXBUF_LOADER);
...
static void capa_thumbnail_loader_get_property(GObject *object,
					       guint prop_id,
					       GValue *value,
					       GParamSpec *pspec)
{
    CapaThumbnailLoader *loader = CAPA_THUMBNAIL_LOADER(object);
    CapaThumbnailLoaderPrivate *priv = loader->priv;
...
static void capa_thumbnail_loader_set_property(GObject *object,
                                               guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec)
{
...
static char *capa_thumbnail_loader_path_to_uri(const char *filename)
{
...
static char *capa_thumbnail_loader_uri_to_md5(const char *uri)
{
...
static char *capa_thumbnail_loader_uri_to_thumb(const char *uri)
{
    char *md5 = capa_thumbnail_loader_uri_to_md5(uri);
    char *thumb;
...
static GdkPixbuf *capa_thumbnail_loader_generate(const char *filename,
                                                 const char *uri,
                                                 ...
                                                 time_t mtime)
{
    ...
    CAPA_DEBUG("Image %dx%d -> %dx%d", iw, ih, tw, th);
...
static GdkPixbuf *capa_thumbnail_loader_pixbuf_load(CapaPixbufLoader *loader,
                                                    const char *filename)
{
    CapaThumbnailLoader *tloader = CAPA_THUMBNAIL_LOADER(loader);
    CapaThumbnailLoaderPrivate *priv = tloader->priv;
    char *uri = capa_thumbnail_loader_path_to_uri(filename);
    char *thumbname = capa_thumbnail_loader_uri_to_thumb(uri);
    GdkPixbuf *result = NULL;
    GdkPixbuf *thumb = NULL;
    struct stat sb;

    if (stat(filename, &sb) < 0) {
        CAPA_DEBUG("File %s does not exist", filename);
        goto cleanup;
    }

    CAPA_DEBUG("Want thumbnail %s for %s %ld", thumbname, uri, sb.st_mtime);
    ...
    if (thumb) {
        ...
        CAPA_DEBUG("Check thumbnail %s for %s %s",
    ...
    if (!thumb) {
        unlink(thumbname);
        CAPA_DEBUG("Generate thumbnail %s for %s %ld",
...
static void capa_thumbnail_loader_class_init(CapaThumbnailLoaderClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);
    CapaPixbufLoaderClass *loader_class = CAPA_PIXBUF_LOADER_CLASS(klass);

    object_class->get_property = capa_thumbnail_loader_get_property;
    object_class->set_property = capa_thumbnail_loader_set_property;

    loader_class->pixbuf_load = capa_thumbnail_loader_pixbuf_load;
    ...
    g_type_class_add_private(klass, sizeof(CapaThumbnailLoaderPrivate));
}
...
CapaThumbnailLoader *capa_thumbnail_loader_new(int width,
                                               int height)
{
    return CAPA_THUMBNAIL_LOADER(g_object_new(CAPA_TYPE_THUMBNAIL_LOADER,
                                              "width", width,
                                              "height", height,
                                              NULL));
}
...
static void capa_thumbnail_loader_init(CapaThumbnailLoader *loader)
{
    CapaThumbnailLoaderPrivate *priv;

    priv = loader->priv = CAPA_THUMBNAIL_LOADER_GET_PRIVATE(loader);
